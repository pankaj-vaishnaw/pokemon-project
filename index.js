...
const filterByTypeButton = document.getElementById("filterByType");

filterByTypeButton.addEventListener("click", (e) => {
  e.preventDefault();
  const selectedType = prompt("Enter a Pokemon type to filter by:");
  if (selectedType) {
    const filteredByType = pokemonDetails.filter((pokemon) =>
    ...
    if (filteredByType.length > 0) {
      ...
      alert(`No Pokemon found with type: ${selectedType}`);
    }
  }
});
